...
/* eslint-env node */
import fs from 'fs';
import { isLargeFile, calculateStats } from '../common/performance-data-processor.js';
 ...
 */
async function generateSummaryReport(processedDataPath, templatePath, outputPath) {
...
// Export functions
export { generateSummaryReport, transformApiData };

// Entry point for CLI usage
if (import.meta.url === import.meta.main) {
  const args = process.argv.slice(2);
  if (args.length < 3) {
    console.error('Usage: node api-summary-report-generator.js <processed-data.json> <template.html> <output.html>');
    process.exit(1);
  }
  ...
  (async () => {
    try {
      await generateSummaryReport(args[0], args[1], args[2]);
      console.log(`Summary report successfully generated: ${args[2]}`);
    } catch (error) {
