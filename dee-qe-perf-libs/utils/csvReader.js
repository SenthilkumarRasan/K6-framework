...
export function parseCsvWithHeaders(csvString) {
  ...
  // Remove possible UTF-8 BOM from the first header and trim each header
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    // Split values and trim; also strip stray CR if present
    const values = line.split(',').map(value => (value || '').replace(/\r/g, '').trim());
    ...
    headers.forEach((header, index) => {
      ...
      obj[key] = value;
      // Create common aliases to tolerate CSV header case/variant differences
