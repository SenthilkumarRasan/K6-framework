...
function extractTransactions(metrics, options = {}) {
  ...
  for (const txName in transactions) {
    ...
    // If we have status codes, use them for success/failure determination
    if (Object.keys(tx.statusCodes).length > 0) {
      tx.failed = tx.failures;
      tx.successful = tx.requests - tx.failures;
    } else {
      // Otherwise, use check-based success/failure rates
      console.log(`Using check-based success/fail rates for transaction ${txName} due to missing status codes`);
      tx.failed = tx.failures;
      tx.successful = tx.requests - tx.failures;
    }
...
if (require.main === module) {
  ...
  try {
    processK6Data(rawMetricsPath, summaryPath, outputPath, testInfo, { filterNonHtml });
  } catch (error) {
    console.error(`Error processing K6 data: ${error.message}`);
    throw error; // Re-throw to allow calling code to handle the error
  }
